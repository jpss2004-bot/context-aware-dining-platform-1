...
BACKUP_DIR="$ROOT/.patch6_single_flow_onboarding_backup_$STAMP"

FILES=(
  "$FRONTEND_DIR/src/pages/OnboardingPage.tsx"
...
cp "$FRONTEND_DIR/src/pages/OnboardingPage.tsx" \
  "$BACKUP_DIR/frontend/src/pages/OnboardingPage.tsx"
cp "$FRONTEND_DIR/src/styles.css" \
  "$BACKUP_DIR/frontend/src/styles.css"

echo "Starting patch6_single_flow_onboarding..."
...
onboarding_path.write_text("""import { useEffect, useMemo, useState } from "react";
...
.single-onboarding-progress-fill {
  ...
  background: linear-gradient(90deg, rgba(125, 156, 102, 0.95), rgba(87, 122, 66, 0.95));
}
...
"""
if ".single-onboarding-shell" not in styles:
    styles += extra
...
echo
echo "Patch 6 applied successfully."
echo "Files changed:"
...
echo "2) open /onboarding or /profile/preferences"
echo "3) confirm the page is now a single guided component"
echo "4) confirm each step clearly says choose one / choose one or more"
echo "5) confirm progress bar advances step by step"
echo "6) confirm there is no side preview or profile-completeness panel"
echo "7) confirm save still works and redirects correctly"
